...
# channel.queue_declare(queue='hello', durable=True)  durable参数为消息持久化，
# rabbitMQ挂掉重启后队列还在，但是队列的消息会被清零，消费者和生产者都要写
...
# channel.basic_qos(prefetch_count=1)  # 类似权重，按能力分发，如果消费者端还存有一个消息未处理，就不在给你发
